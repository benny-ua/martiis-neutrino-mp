...
int CChannelList::show()
{
	...
	while (loop) {
		...
		else if (msg == CRCInput::RC_up || (int) msg == g_settings.key_pageup)
		{
			...
			if (new_selected < 0)
				new_selected = (selected == 0) ? chanlist.size() - 1 : 0;
...
void CChannelList::paintDetails(int index)
{
	...
	if (IS_WEBTV(chanlist[index]->channel_id)) {
		g_Font[SNeutrinoSettings::FONT_TYPE_CHANNELLIST]->RenderString(x+ 10, y+ height+ 5+ fheight,                  full_width - 30, chanlist[index]->getDesc(), colored_event_C ? COL_COLORED_EVENTS_TEXT : COL_MENUCONTENTDARK_TEXT);
		g_Font[SNeutrinoSettings::FONT_TYPE_CHANNELLIST]->RenderString(x+ 10, y+ height+ 5+ 2*fheight + fdescrheight, full_width - 30, chanlist[index]->getUrl(), COL_MENUCONTENTDARK_TEXT);
		...
		g_Font[SNeutrinoSettings::FONT_TYPE_CHANNELLIST]->RenderString(x+ 10, y+ height+ 5+ 2*fheight +fdescrheight, full_width - 30, desc, COL_MENUCONTENTDARK_TEXT);
	}
...
//					g_Font[SNeutrinoSettings::FONT_TYPE_IMAGEINFO_SMALL]->RenderString(x+ 5+ numwidth+ 6, ypos+ xtheight, width- numwidth- 20- 15 -poffs, tmp, COL_MENUCONTENT_TEXT);
			...
			if (g_settings.colored_events_channellist == 1) // current event color
