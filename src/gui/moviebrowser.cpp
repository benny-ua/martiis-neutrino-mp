...
int CMovieBrowser::showMovieInfoMenu(MI_MOVIE_INFO* movie_info)
{
    ...
    for(int li =0 ; li < MI_MOVIE_BOOK_USER_MAX && li < MAX_NUMBER_OF_BOOKMARK_ITEMS; li++ )
    {
        ...
        bookmarkMenu.addItem( new CMenuForwarder (movie_info->bookmarks.user[li].name.c_str(),   true, NULL,pBookItemMenu[li]));
    }
...
#define BUFFER_SIZE 100
    ...
    movieInfoMenu.addItem( new CMenuForwarder(LOCALE_MOVIEBROWSER_INFO_PREVPLAYDATE,   false, dateUserDateInput.getValueString()));//LOCALE_FLASHUPDATE_CURRENTVERSIONDATE
    movieInfoMenu.addItem( new CMenuForwarder(LOCALE_MOVIEBROWSER_INFO_RECORDDATE,     false, recUserDateInput.getValueString()));//LOCALE_FLASHUPDATE_CURRENTVERSIONDATE
