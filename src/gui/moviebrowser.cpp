...
void CMovieBrowser::refreshYTMenu()
{
	...
	if (!yt_pending.empty()) {
		...
		for (std::vector<MI_MOVIE_INFO>::iterator it = yt_pending.begin(); it != yt_pending.end(); ++it, ++i) {
			yt_menue->addItem(new CMenuForwarderNonLocalized((*it).file.Name.c_str(), true, NULL, ytcache_selector));
		}
	}
...
bool CMovieBrowser::showYTMenu()
{
	...
	CStringInputSMS stringInput(LOCALE_MOVIEBROWSER_YT_SEARCH, &search, 30, NONEXISTANT_LOCALE, NONEXISTANT_LOCALE, "abcdefghijklmnopqrstuvwxyz0123456789-+./ ");
