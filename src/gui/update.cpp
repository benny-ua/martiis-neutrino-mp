...
bool CFlashExpert::checkSize(int mtd, std::string &backupFile)
{
	...
	printf("##### [%s] backupMaxSize: %llu, btotal: %llu, bused: %llu, bsize: %ld\n", __FUNCTION__, (long long unsigned) backupMaxSize, (long long unsigned) btotal, (long long unsigned) bused, bsize);
...
int CFlashExpert::showFileSelector(const std::string & actionkey)
{
	...
	else
	{
		for(int count=0;count<n;count++)
		{
			...
			if(pos!=-1)
			{
				fileselector->addItem(new CMenuForwarder(filen, true, NULL, this, (actionkey + filen).c_str()));
