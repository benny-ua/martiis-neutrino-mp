...
int CSubChannelSelectMenu::getNVODMenu(CMenuWidget* menu)
{
	...
	for ( CSubServiceListSorted::iterator e=g_RemoteControl->subChannels.begin(); e!=g_RemoteControl->subChannels.end(); ++e)
	{
		...
		if ( !g_RemoteControl->are_subchannels ) 
		{
			...
			menu->addItem(new CMenuForwarder(std::string(nvod_s), enabled, NULL, &NVODChanger, nvod_id), (count == g_RemoteControl->selected_subchannel));
		} 
		else 
		{
			menu->addItem(new CMenuForwarder(e->subservice_name, enabled, NULL, &NVODChanger, nvod_id, CRCInput::convertDigitToKey(count)), (count == g_RemoteControl->selected_subchannel));
		}
