...
int CScanSetup::showScanMenuFrontendSetup()
{
	...
	if (CFEManager::getInstance()->haveSat()) {
		...
		mf = new CMenuDForwarder(LOCALE_EXTRA_LATITUDE, true, NULL, toff1, "", CRCInput::convertDigitToKey(shortcut++));
		...
		mf = new CMenuDForwarder(LOCALE_EXTRA_LONGITUDE, true, NULL, toff2, "", CRCInput::convertDigitToKey(shortcut++));
		mf->setHint("", LOCALE_MENU_HINT_SCAN_LONGITUDE);
		rotorMenu->addItem(mf);

		CMenuOptionNumberChooser * nc = new CMenuOptionNumberChooser(LOCALE_SATSETUP_USALS_REPEAT, (int *)&zapitCfg.repeatUsals, true, 0, 10, NULL);
...
void CScanSetup::addScanMenuManualScan(CMenuWidget *manual_Scan, bool stest)
{
	...
	if (r_system == DVB_C)  {
		...
		CMenuOptionStringChooser * terrSelect = new CMenuOptionStringChooser(LOCALE_TERRESTRIALSETUP_PROVIDER, &scansettings.terrName, true, this, CRCInput::RC_red, NULL, true);
